...
def compare_baseline_algos(csr: sp.csr_matrix, n_cores: int = None, warmup: int = 1, runs: int = 3) -> Tuple[List[Dict], int]:
    ...
    try:
        # Python's lzma module doesn't expose threading, use xz via subprocess if available
        if shutil.which("xz"):
            def compress_xz(data):
                ...
                try:
                    subprocess.run(
                        ["xz", "-9", "--extreme", f"-T{n_cores}", "-c"],
                        stdin=open(temp_in, "rb"),
                        stdout=open(temp_out, "wb"),
                        check=True,
            ...
            def decompress_xz(compressed):
                with tempfile.NamedTemporaryFile(delete=False, suffix=".xz") as f:
                    temp_in = f.name
                    f.write(compressed)
                try:
    ...
    # 5. zstd (level 22 - maximum, multi-threaded)
    ...
    try:
        ...
        print("    Skipped: zstandard not installed (pip install zstandard)")
    except Exception as e:
        print(f"    Failed: {e}")
...
def save_results_to_csv(results: List[Dict], metadata: Dict, output_path: str) -> None:
    ...
    metric_cols = ["method", "compressed_size", "original_size", "compression_ratio", 
  ...
  python compress_h5ad.py input.h5ad --compare-algos --no-csv
    ...
    for input_path in inputs:
        ...
        if args.compare_algos:
            ...
            print(f"    Size: {cellz_result['compressed_size']:,} bytes, Ratio: {cellz_result['compression_ratio']:.3f}x, Time: {cellz_result['compress_time_mean']*1000:.1f}ms, Bits/NNZ: {bpnz:.2f}")
            ...
            for r in sorted(all_results, key=lambda x: x['compressed_size']):
                ...
                # Efficiency ratio (can be >100% if compressor beats naive bound)
                efficiency = (theoretical_min / bits_per_nnz * 100) if bits_per_nnz > 0 else 0
                print(f"{r['method']:<20} {r['compressed_size']:<15,} {r['compression_ratio']:<10.3f} {bits_per_nnz:<12.2f} {efficiency:<10.1f} {comp_time:<15} {decomp_time:<15}")
